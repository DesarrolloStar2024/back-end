...
export function parseCadena(cadena) {
    if (!cadena)
        return {};
...
export function parseBoolish(v) {
    ...
    if (["s", "si", "sí", "true", "1"].includes(s))
        return "S";
    if (["n", "no", "false", "0"].includes(s))
        return "N";
    return undefined;
}
function singularPluralVariants(tok) {
    const set = new Set([tok]);
    if (tok.endsWith("es"))
        set.add(tok.slice(0, -2));
    if (tok.endsWith("s"))
        set.add(tok.slice(0, -1));
    set.add(tok + "s");
    set.add(tok + "es");
    return [...set];
}
export async function expandWithSynonyms(tokens, SynonymModel) {
    if (!tokens.length)
        return [];
    const rows = await SynonymModel.find({ term: { $in: tokens } }).lean();
    const map = new Map();
    rows.forEach((r) => map.set(r.term, r.synonyms || []));
    const expanded = new Set();
    tokens.forEach((t) => {
        singularPluralVariants(t).forEach((v) => expanded.add(v));
        (map.get(t) || []).forEach((s) => singularPluralVariants(s).forEach((v) => expanded.add(v)));
    });
    return [...expanded];
}
export function buildOrRegex(fields, terms) {
    const ors = terms.map((t) => new RegExp(t.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"));
    return {
