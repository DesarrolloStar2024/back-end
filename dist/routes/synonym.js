...
synonymsRoute.get("/", async (c) => {
    ...
    const size = Math.min(1000, Math.max(1, parseInt(c.req.query("size") || "50", 10)));
...
synonymsRoute.post("/", async (c) => {
    ...
    const synonyms = (b.synonyms || [])
        ...
        .filter(Boolean);
    if (!term)
        return c.json({ message: "term requerido" }, 400);
...
synonymsRoute.patch("/:term", async (c) => {
    ...
    const ops = {};
    if (add.length)
        ops.$addToSet = { synonyms: { $each: add } };
    if (remove.length)
        ops.$pull = { synonyms: { $in: remove } };
...
synonymsRoute.post("/upsert", async (c) => {
    ...
    for (const row of body) {
        ...
        if (!term)
            continue;
        await Synonym.updateOne({ term }, { $set: { term, synonyms } }, { upsert: true });
        upserts++;
    }
