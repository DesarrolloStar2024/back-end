...
superAdminsRoute.get("/", async (c) => {
    ...
    if (codigosCsv) {
        const codigos = codigosCsv
            ...
            .filter(Boolean);
        if (codigos.length)
            filter.$and.push({ Codigo: { $in: codigos } });
    }
...
superAdminsRoute.put("/:codigo", authMiddleware(true), async (c) => {
    ...
    if (body?.Id !== undefined)
        upd.Id = String(body.Id).trim();
    if (body?.Codigo !== undefined)
        upd.Codigo = String(body.Codigo).trim(); // opcional: cambiar clave
...
superAdminsRoute.post("/upsert", async (c) => {
    ...
    await Promise.all(arr.map(async (row) => {
        ...
        if (!Codigo || !Id)
            return;
        await SuperAdmin.updateOne({ Codigo }, { $set: { Codigo, Id } }, { upsert: true });
        upserts++;
    }));
