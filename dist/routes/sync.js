...
const toNumStr = (v) => (v == null ? "" : String(v));
...
function normalizeExistencias(remote) {
    return (remote ?? []).map((e) => ({
...
// ====== CORE WRITERS ======
async function writeProducts(products, batchSize, onProgress) {
    let done = 0;
...
async function writePrices(items, batchSize, onProgress) {
    let done = 0;
...
async function writeStocks(items, batchSize, onProgress) {
    ...
    for (const group of chunk(items, batchSize)) {
        const ops = group
            ...
            if (!Codigo)
                return null;
            const Existencias = normalizeExistencias(row.Existencias ?? row.existencias ?? []);
            return {
                updateOne: {
                    filter: { Codigo },
...
syncRoute.post("/full", authMiddleware(true), async (c) => {
    const { codes, size = 5000, batchSize = 800, } = await c.req.json().catch(() => ({}));
    let total = 0;
...
syncRoute.post("/stock", authMiddleware(true), async (c) => {
    const { codes, fecha, batchSize = 1000, } = await c.req.json().catch(() => ({}));
    let acc = [];
    if (Array.isArray(codes) && codes.length) {
        for (const code of codes) {
        ...
        try {
            ...
            if (!all || all.length === 0) {
                await stream.writeSSE({
                    ...
                    data: JSON.stringify({
                        phase: "error",
                        message: "Sysplus no respondió o devolvió 0 productos. No se aplicaron cambios.",
                    }),
                });
                await stream.writeSSE({
                    event: "done",
