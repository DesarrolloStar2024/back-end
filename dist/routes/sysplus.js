...
sysplusRoute.post("/cotizacion/log", async (c) => {
    try {
        ...
        if (!requestBody || !sysplusResponse) {
            return c.json({ ok: false, message: "Faltan datos para registrar log." }, 400);
        }
        // ✅ Validar si la respuesta es exitosa
        const isValid = sysplusResponse?.RESP &&
        ...
        if (isValid && Array.isArray(requestBody.ITEMS)) {
            for (const item of requestBody.ITEMS) {
                ...
                if (!barras || isNaN(cantidad) || cantidad <= 0)
                    continue;
                // Buscar producto
                ...
                const bodegaAlt = product.Existencias.find((e) => e.Bodega !== "01" && e.Bodega !== "06");
                ...
                await product.save();
                console.log(`📦 Stock actualizado: ${barras} (${target.Bodega}) ${actual} → ${nuevo}`);
                movimientos.push({
                    barras,
...
sysplusRoute.get("/cotizacion/logs", authMiddleware(true), async (c) => {
    try {
        ...
        if (vendedor)
            filter.vendedor = Number(vendedor);
        if (cliente)
            filter.cliente = { $regex: cliente, $options: "i" }; // búsqueda parcial
