...
productsRoute.get("/", async (c) => {
    await connectDB();
    // -------- Helpers ----------
    const toBool = (v) => v != null &&
        ["true", "1", "s", "si", "sí", "y", "yes"].includes(String(v).toLowerCase());
    const csv = (s) => s
        ? s
            .split(",")
            .map((x) => x.trim())
    ...
    if (cadena) {
        ...
        const jerarquia = {};
        if (parsed.CodFami)
            jerarquia.CodFami = parsed.CodFami;
        if (parsed.CodGrupo)
            jerarquia.CodGrupo = parsed.CodGrupo;
        if (parsed.CodSubgrupo)
            jerarquia.CodSubgrupo = parsed.CodSubgrupo;
        ...
        const jerarquia = {};
        if (codFami)
            jerarquia.CodFami = codFami;
        if (codGrupo)
            jerarquia.CodGrupo = codGrupo;
        if (codSubgrupo)
            jerarquia.CodSubgrupo = codSubgrupo;
        if (Object.keys(jerarquia).length) {
            baseAnd.push(jerarquia);
        }
        // Si no hay ninguno, no aplica filtro de jerarquía
    ...
    if (codigo)
        baseAnd.push({ Codigo: String(codigo) });
    if (barras)
        baseAnd.push({ Barras: String(barras) });
    ...
    if (marcaId)
        baseAnd.push(byMarcaIdStrict(String(marcaId).trim()));
    if (fabricanteId)
        baseAnd.push(byFabricanteIdStrict(String(fabricanteId).trim()));
    if (desta)
        baseAnd.push({ Desta: desta });
    if (masve)
        baseAnd.push({ Masve: masve });
    if (nuevo)
        baseAnd.push({ Nuevo: nuevo });
    if (promo)
        baseAnd.push({ Promo: promo });
    if (hasPromoCatalogo) {
    ...
    const maxCantidad = numOr(c.req.query("maxCantidad")) ?? numOr(c.req.query("cantLe")); // alias cantLe
...
productsRoute.get("/:codigo/suggest", async (c) => {
    ...
    const baseDesc = base && typeof base === "object" && !Array.isArray(base)
    ...
    const broadOr = [];
    if (baseFami)
        broadOr.push({ CodFami: baseFami });
    if (baseGrupo)
        broadOr.push({ CodGrupo: baseGrupo });
    if (baseSub)
        broadOr.push({ CodSubgrupo: baseSub });
    if (baseMarca)
        broadOr.push({ Marca: baseMarca });
    if (baseFab)
        broadOr.push({ Fabricante: baseFab });
    if (orRegex?.$or?.length)
        broadOr.push(...orRegex.$or);
    pipeline.push({
        $match: {
            Codigo: { $ne: codigo },
...
// --- Helpers ---
const toBoolish = (v) => typeof v === "boolean"
    ? v
    : ["true", "1", "s", "si", "sí", "y", "yes"].includes(String(v).toLowerCase());
...
productsRoute.patch("/:codigo/ref-catalogo", authMiddleware(true), async (c) => {
    await connectDB();
    const codigo = c.req.param("codigo");
    let body;
    try {
    ...
    const res = await Product.updateOne({ Codigo: codigo }, { $set: { RefCatalogo: value } });
    return c.json({
        ok: true,
        codigo,
        value,
...
productsRoute.post("/ref-catalogo/bulk", authMiddleware(true), async (c) => {
    ...
    if (!codes.length)
        return c.json({ error: "Debe enviar 'codes'." }, 400);
    if (body?.value === undefined)
        return c.json({ error: "Falta 'value' (boolean)" }, 400);
    const value = toBoolish(body.value);
    const res = await Product.updateMany({ Codigo: { $in: codes } }, { $set: { RefCatalogo: value } });
    return c.json({
        ok: true,
...
productsRoute.patch("/:codigo/promo-catalogo", authMiddleware(true), async (c) => {
    ...
    if (!hasActivo && !hasPromo) {
        return c.json({ error: "No hay campos válidos (use 'activo' y/o 'promo')." }, 400);
    }
    // Traer el actual para mergear si es objeto
    const doc = await Product.findOne({ Codigo: codigo }, { PromoCatalogo: 1 }).lean();
    // Normalizar base
    let base = { activo: false, promo: "" };
    if (doc &&
        !Array.isArray(doc) &&
        doc.PromoCatalogo &&
    ...
    const res = await Product.updateOne({ Codigo: codigo }, { $set: { PromoCatalogo: nuevo } });
    return c.json({
        ok: true,
        codigo,
...
productsRoute.post("/promo-catalogo/bulk", authMiddleware(true), async (c) => {
    ...
    if (!items.length) {
        return c.json({ error: "Debe enviar 'items' o 'text' con código y promo." }, 400);
    }
